Performance_Eva <- function(train.data, 
                            test.data, 
                            beta.est.train,
                            para.all,
                            family)
{
	...
	size.train <- nrow(train.data$data.x[[1]])
  ...
  mu_bar <- median(cbind(1,train.data$data.merge[[1]][,-1])%*%para.all)
	bias.me <- sum((mu_bar-rowSums(train.data$data.x[[1]]*train.data$beta.true[[1]]))^2)/size.train
	var.me <- sum((cbind(1,train.data$data.merge[[1]][,-1])%*%para.all-mu_bar)^2)/size.train
