...
kl.loss.train <- array(NA,dim=c(s.cnt, rep, 9))
kl.loss.test <- array(NA,dim=c(s.cnt, rep, 9))
er.train <- array(NA,dim=c(s.cnt, rep, 8))
er.test <- array(NA,dim=c(s.cnt, rep, 8))
weight.mat <- array(NA,dim=c(s.cnt, rep, s.cnt))
weight.mat20 <- array(NA,dim=c(s.cnt, rep, s.cnt))
weight.matjcv <- array(NA,dim=c(s.cnt, rep, s.cnt))
time.consume <- array(NA,dim=c(s.cnt, rep, 8))
...
for(h0 in 1:s.cnt){
  ...
  for(r in 1:rep){
    ...
    time.consume[h0,r,1]=as.double(difftime(Sys.time(), timeES, units="secs"))+time.consume.sp
    ...
    time.consume[h0,r,2]=as.double(difftime(Sys.time(), timeES20, units="secs"))+time.consume.sp
    ...
    weight.mat20[h0,r,] <- weight.est20
    ...
    time.consume[h0,r,3]=as.double(difftime(Sys.time(), timeESn, units="secs"))+time.consume.sp
    ...
    weight.matjcv[h0,r,] <- weight.estjcv
    ...
    for(k in 1:s.cnt){
      ...
      gvcm.res.sa[[k]] <- glm(respon~., data = data.train.frame.sa, family=family, control=list(maxit=1000))
    ...
    time.consume[h0,r,4]=as.double(difftime(Sys.time(), timeSA, units="secs"))
    ...
    gvcm.res.tar <- glm(respon~., data = data.train.frame.tar, family=family, control=list(maxit=1000))
    ##
    para.tar <- gvcm.res[[1]]$coefficients
    time.consume[h0,r,5]=as.double(difftime(Sys.time(), timeTAR, units="secs"))
    ...
    time.consume[h0,r,6]=as.double(difftime(Sys.time(), timePMLE, units="secs"))
    ...
    time.consume[h0,r,7]=as.double(difftime(Sys.time(), timeTransGLM, units="secs"))
    ...
    if(family == 'gaussian'){
      timeTransL=Sys.time()
      X.comb <- cbind(do.call(rbind, data.train$data.x), do.call(rbind, data.train$data.u))
      y.comb <- as.vector(do.call(cbind, data.train$data.y))
      ...
      para.tlasso <- (prop.re1$beta.hat + prop.re2$beta.hat) / 2
      time.consume[h0,r,8]=as.double(difftime(Sys.time(), timeTransL, units="secs"))
      pred.tlasso.train <- cbind(matrix(rep(1,size.train[1]),size.train[1],1), data.train.adj$data.merge[[1]][,-1])%*%para.tlasso
      kl.loss.train[h0,r,8] <- -2*sum(pred.tlasso.train*data.train.adj$data.y[[1]]-log(1+exp(pred.tlasso.train)))/size.train[1]
      pred.tlasso.test <- cbind(matrix(rep(1,size.test[1]),size.test[1],1), data.test.adj$data.merge[,-1])%*%para.tlasso
      kl.loss.test[h0,r,8] <- -2*sum(pred.tlasso.test*data.test.adj$data.y-log(1+exp(pred.tlasso.test)))/size.test[1]
    }
    ...
    time.consume[h0,r,9]=as.double(difftime(Sys.time(), timeART, units="secs"))
    
    pred.art.train <- cbind(matrix(rep(1,size.train[1]),size.train[1],1), data.train.adj$data.merge[[1]][,-1])%*%para.all.artlog
    kl.loss.train[h0,r,9] <- -2*sum(pred.art.train*data.train.adj$data.y[[1]]-log(1+exp(pred.art.train)))/size.train[1]
    pred.art.test <- cbind(matrix(rep(1,size.test[1]),size.test[1],1), data.test.adj$data.merge[,-1])%*%para.all.artlog
    kl.loss.test[h0,r,9] <- -2*sum(pred.art.test*data.test.adj$data.y-log(1+exp(pred.art.test)))/size.test[1]
...
weight_assign <- NULL
for(i in 1:3){
  weight_assign_tmp <- round(apply(weight.mat[i,,],2,mean,na.rm=TRUE),2)
  weight_assign <- c(weight_assign, weight_assign_tmp)
}

source_ind = c("20-30", "30-50", "50+", "30-50", "20-30", "50+", "50+", "20-30", "30-50")
tar_ind = rep(source.ind, each=3)
data_total = data.frame(weight_assign=weight_assign, source_ind=source_ind, tar_ind=tar_ind)
data_total$tar_ind <- factor(data_total$tar_ind, levels = c("20-30", "30-50", "50+"))
...
ggplot(data=data_total, 
  ...
  ylab('Weight assignment') +
  xlab('Target domain') +
  scale_fill_manual(values = c("20-30"="#98d09d","30-50"="#fbf398","50+"="#e77381"),
                    limits=c("20-30", "30-50", "50+")) +
...
for(hh in 1:length(source.ind)){
  kl_loss_fig_train <- c(kl_loss_fig_train, as.vector(kl.loss.train[hh,,-8]))
  kl_loss_fig_test <- c(kl_loss_fig_test, as.vector(kl.loss.test[hh,,-8]))
