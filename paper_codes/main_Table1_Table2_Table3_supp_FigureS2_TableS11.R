...
beta.est.tloap10 <- array(NA,dim=c(length(size.cnd), length(family), rep, p-qz[1]))
beta.est.tloap5 <- array(NA,dim=c(length(size.cnd), length(family), rep, p-qz[1]))
beta.est.tloapj <- array(NA,dim=c(length(size.cnd), length(family), rep, p-qz[1]))
beta.est.sma <- array(NA,dim=c(length(size.cnd), length(family), rep, p-qz[1]))
beta.est.tar <- array(NA,dim=c(length(size.cnd), length(family), rep, p-qz[1]))
beta.est.pool <- array(NA,dim=c(length(size.cnd), length(family), rep, p-qz[1]))
beta.est.glmtrans <- array(NA,dim=c(length(size.cnd), length(family), rep, p-qz[1]))
beta.est.tlasso <- array(NA,dim=c(length(size.cnd), length(family), rep, p-qz[1]))
beta.est.art <- array(NA,dim=c(length(size.cnd), length(family), rep, p-qz[1]))
...
for(i0 in 1:length(size.cnd)){
  ...
  for(i in 1:length(family)){
    ...
    for(r in 1:rep){
      ...
      beta.est.tloap10[i0,i,r,] <- beta.est.train
      ...
      beta.est.tloap5[i0,i,r,] <- beta.est.train20
      ...
      beta.est.tloapj[i0,i,r,] <- beta.est.trainjcv
      ...
      beta.est.tar[i0,i,r,] <- beta.tar
      
      
      ###### MLE-Pooled #######
      timePMLE=Sys.time()
      data.train.adj <- data.train
      for(ii in 1:length(mis.source.index)){
        data.train.adj$data.x[[mis.source.index[ii]]] <- data.train.adj$data.x[[mis.source.index[ii]]][,-(p+1)]
      }
      # nknots.all <- ceiling(sum(size)^(1/5))
      # ndf.all <- ndg+nknots.all
      ...
      datay.all <- as.vector(do.call(cbind, data.train.adj$data.y))
      ...
      beta.est.pool[i0,i,r,] <- beta.est.all
      ...
      data.test.adj$data.merge <- cbind(data.test.adj$data.y, data.test.adj$data.x[,1:p])
      data.train.adj$data.merge[[1]] <- cbind(data.train.adj$data.y[[1]], data.train.adj$data.x[[1]][,1:p])
      ...
      res.glmtrans <- Performance_Eva(train.data = data.train.adj, test.data = data.test.adj,  beta.est.train = beta.glmtrans, para.all = para.all.tian, family = family[i])
      ...
      beta.est.glmtrans[i0,i,r,] <- beta.glmtrans
      ...
      if(family[i] == 'gaussian'){
        ...
        res.translasso <- Performance_Eva(train.data = data.train.adj, test.data = data.test.adj,  beta.est.train = beta.translasso[(qz[1]+2):(p+1)], para.all = beta.translasso, family = family[i])
        ...
        beta.est.tlasso[i0,i,r,] <- beta.translasso[(qz[1]+2):(p+1)]
      }
      ...
      if(family[i] %in% c('gaussian','binomial')){
        ...
        if(family[i]=="binomial"){
          fit.art.log <- ART(target.data$x, target.data$y, source.data.x, source.data.y, data.test.adj$data.x, func=fit_logit, type="classification")
          beta.art.log <- fit.art.log$coef_ART[(qz[1]+2):(p+1)]
          para.all.artlog <- fit.art.log$coef_ART
          res.art <- Performance_Eva(train.data = data.train.adj, test.data = data.test.adj,  beta.est.train = beta.art.log, para.all = para.all.artlog, family = family[i])
          beta.est.art[i0,i,r,] <- beta.art.log
        }else{
          fit.art.lm <- ART(target.data$x, target.data$y, source.data.x, source.data.y, data.test.adj$data.x, func=fit_lm, type="regression")
          beta.art.lm <- fit.art.lm$coef_ART[(qz[1]+2):(p+1)]
          para.all.artlm <- fit.art.lm$coef_ART
          res.art <- Performance_Eva(train.data = data.train.adj, test.data = data.test.adj,  beta.est.train = beta.art.lm, para.all = para.all.artlm, family = family[i])
          beta.est.art[i0,i,r,] <- beta.art.lm
        }
