...
time.consume <- array(NA,dim=c(s.cnt, rep, 7))
...
for(h0 in 5:6){ 
  ...
  for(r in 4:rep){
    ...
    time.consume[h0,r,3]=as.double(difftime(Sys.time(), timeSA, units="secs"))
    ...
    time.consume[h0,r,4]=as.double(difftime(Sys.time(), timeTAR, units="secs"))
    ...
    time.consume[h0,r,5]=as.double(difftime(Sys.time(), timePMLE, units="secs"))
    ...
    time.consume[h0,r,6]=as.double(difftime(Sys.time(), timeTransGLM, units="secs"))
    ...
    if ('try-error' %in% class(fit.art.log)){
      ...
      time.consume[h0,r,7]=as.double(difftime(Sys.time(), timeART, units="secs"))
...
method_name = rep(c(rep('TLOAP-CV10', rep), rep('TLOAP-CV5', rep),
                    rep('TLAP-EW', rep), rep('MLE-Target', rep), 
                    rep('MLE-Pooled', rep), rep('Trans-GLM', rep), rep('ART', rep)), 2)
legend_name = c(rep("E",rep*7), rep("F",rep*7))

data_total_kl_train = data.frame(kl_loss=kl_loss_fig_train, Methods=method_name, Target = legend_name)
data_total_kl_train$Methods <- factor(data_total_kl_train$Methods, levels = c("TLOAP-CV10", "TLOAP-CV5", "TLAP-EW", "MLE-Target", "MLE-Pooled", "Trans-GLM","ART"))
data_total_kl_train$Target <- factor(data_total_kl_train$Target, levels = c("E", "F"))

data_total_kl_test = data.frame(kl_loss=kl_loss_fig_test, Methods=method_name, Target = legend_name)
data_total_kl_test$Methods <- factor(data_total_kl_test$Methods, levels = c("TLOAP-CV10", "TLOAP-CV5", "TLAP-EW", "MLE-Target", "MLE-Pooled", "Trans-GLM","ART"))
data_total_kl_test$Target <- factor(data_total_kl_test$Target, levels = c("E", "F"))

data_total_er_train = data.frame(er=er_fig_train, Methods=method_name, Target = legend_name)
data_total_er_train$Methods <- factor(data_total_er_train$Methods, levels = c("TLOAP-CV10", "TLOAP-CV5", "TLAP-EW", "MLE-Target", "MLE-Pooled", "Trans-GLM","ART"))
data_total_er_train$Target <- factor(data_total_er_train$Target, levels = c("E", "F"))

data_total_er_test = data.frame(er=er_fig_test, Methods=method_name, Target = legend_name)
data_total_er_test$Methods <- factor(data_total_er_test$Methods, levels = c("TLOAP-CV10", "TLOAP-CV5", "TLAP-EW", "MLE-Target", "MLE-Pooled", "Trans-GLM","ART"))
data_total_er_test$Target <- factor(data_total_er_test$Target, levels = c("E", "F"))
...
p1 <- ggplot(data=data_total_kl_train, aes(x=Methods, y=kl_loss,color=Methods)) + 
  ...
  facetted_pos_scales(y = list(Target == "E" ~
                                 scale_y_continuous(limits=c(0.6,1.2),breaks=seq(0.6,1.2,0.2)),
                               Target == "F" ~
                                 scale_y_continuous(limits=c(0.8,1.8),breaks=seq(0.8,1.8,0.4))))
...
p2 <- ggplot(data=data_total_kl_test, aes(x=Methods, y=kl_loss,color=Methods)) + 
  ...
  theme(strip.text.x = element_text(size = 40),
        ...
        plot.title = element_text(size = 40)) +
  facetted_pos_scales(y = list(Target == "E" ~
                                 scale_y_continuous(limits=c(0.5,1.2),breaks=seq(0.5,1.2,0.2))))
